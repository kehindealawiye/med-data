...
if 'TOTAL CONTRACT SUM EDITED' in df.columns:
    df['TOTAL CONTRACT SUM EDITED'] = pd.to_numeric(df['TOTAL CONTRACT SUM EDITED'], errors='coerce')
...
@app.callback(
    ...
    Output('kpi-total-contract-sum', 'children'),
    Input('sector-filter', 'value'),
    Input('mda-filter', 'value'),
    Input('year-filter', 'value')
)
def update_dashboard(sector, mda, year):
    filtered_df = df.copy()
    ...
    total_contract_sum = (
        f"₦{filtered_df['TOTAL CONTRACT SUM EDITED'].sum():,.0f}"
        if 'TOTAL CONTRACT SUM EDITED' in filtered_df.columns else "₦0"
    )
